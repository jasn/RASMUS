 ...
 * This window graphically contains a terminal and an environment.
 * This window is responsible for communicating with the rasmus Interpreter.
 */
...
public:
	...
	Ui::MainWindow ui; /** The class generated by Qt from main.ui*/

	Interpreter *interpreter; /** The GUI interpreter, responsible for communication between RASMUS and GUI*/
	QThread interpreterThread; /** A Qt thread for running the interpreter. */
	Settings s; /** User settings. See the Settings class for more info.*/
	...
	MainWindow() {
		...
		// it is beneficial to run the interpreter in a separate thread.
	...
	/**
	 * @Brief Destructor for MainWindow is called when closing the RASMUS gui.
	 */
	~MainWindow() {
...
public slots:
	/**
	 * \brief
	 * This function is called whenever the path used to locate rasmus database (.rdb) files is changed.
	 *
	 * @param s is a Settings object where the users settings is stored. See the Settings class for more info.
	 */
	...
	/**
	 * \brief This function opens a TableViewWindow with the data drawn from the relation r.
	 * @param r is a RASMUS relation.
	 */
	...
	/**
	 * @brief Selecting 'File->Delete Relation' in The main window executes this function.
	 * The function opens a dialog where one can enter the name of a relation, and that relation will then be deleted if it exists.
	 * If the entered name is not a relation nothing happens.
	 */
	void deleteRelation() {
	...
	/**
	 * \brief
	 * Selecting 'File->New' in the GUI executes this function.
	 * The GUI contains an editor for RASMUS code. This function opens that editor with an empty file.
	 */
	void newFile() {
	...
	/**
	 * \brief
	 * This function is executed when the user selects Edit->Preferences and shows the preferences window.
	 */
	...
	/**
	 * \brief
	 * In the GUI we can import relations either from csv files or rdb (rasmus database) files.
	 * This function is exeucted when the user clicks File->Import Relation.
	 * The function opens a dialog where the user can locate a csv or rdb file to be loaded into the RASMUS system.
	 */
	void importRelation() {
	...
	/**
	 * \brief
	 * The environment of RASMUS is a list of variables in the right hand side of the main
	 * graphical user interface. When a variable is double clicked, this function is executed.
	 * If the variable is a relation, then the relation window is open, otherwise nothing happens.
	 */
	...
	/**
	 * \brief
	 * Whenever a global variable is set, unset, or changed this function needs to be called with
	 * the name of the variable that was changed.
	 * The function updates the environment view in the right side of the Main GUI window.
	 *
	 * @param name is a QString with the name of the updated variable.
	 */
	...
	/**
	 * \brief
	 * The 'File' menu has an 'Open File' option, where we can load RASMUS code files.
	 * This function opens the Editor Window with the selected file loaded.
	 */
	void openFile() {
	...
	/**
	 * \brief
	 * Executes the contents of the selected file. The selected file should contain RASMUS code.
	 */
	void runFile() {
...
signals:
	/**
	 * \brief This signal is emitted as the last step in executing runFile().
	 * The signal is received by the slot Interpreter::runContent(QString, QString).
	 */
	void doRunContent(QString, QString);

	/**
	 * \brief This signal is emitted as the last step in executing deleteRelation().
	 * The signal is received by the slot Interpreter::unset(QString).
	 */
	void unset(QString);
};
