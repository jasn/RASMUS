...
void result_manip(teststream & s, bool success) {
	...
	if (s.do_time) {
		...
		s << " " << std::chrono::duration_cast<std::chrono::milliseconds>(ntime - s.time).count() << " ms";
		s.time = ntime;
	}
	s << std::endl;
...
void tests::build_information(std::ostream & o) {
	o << "No version information" << std::flush;
}	
...
namespace bits {
	...
	test_runner::~test_runner() {
		t->end_test(result, 
					t->do_time?(
						std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - m_time).count()):0);
	}
